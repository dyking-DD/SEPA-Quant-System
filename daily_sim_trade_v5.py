...
# ==================== 综合评分 v5.1（7因子模型+VCP） ====================
def calculate_score_v5(stock, tech_data=None, vcp_data=None):
    """
    v5.1综合评分（7因子模型 + VCP）
    1. 资金流向评分 20%  — 涨幅位置
...
def screen_stocks(log):
    ...
    # 获取技术数据（MA/RSI/动量）
    log(f'📈 获取技术数据 + VCP形态（前{len(top_candidates)}只）...')
    ...
    for i, c in enumerate(filtered[:15]):
        ...
        vcp_flag = '✅VCP' if vcp.get('is_vcp') else ('🚀突破' if vcp.get('is_breakout') else '')
